...
// Extended with operator== for libsxx.
//
// See also:
...
public:
    ...
    /// For stack storage, compares bytes.
    /// For dynamic storage, compares pointers.
    bool operator==(any const& rhs) const {
        return (type() == rhs.type()) &&
            ((empty() && rhs.empty()) ||
             (!empty() && vtable->equals(this->storage, rhs.storage)));
...
private: // Storage and Virtual Method Table
    ...
    struct vtable_type
    {
        ...
        bool(*equals)(const storage_union& lhs, const storage_union &rhs) noexcept;
    };
    ...
    struct vtable_dynamic
    {
        ...
        static bool equals(const storage_union& lhs, const storage_union &rhs) noexcept {
    ...
    struct vtable_stack
    {
        ...
        static bool equals(const storage_union& lhs, const storage_union &rhs) noexcept {
            return std::memcmp(&lhs.stack, &rhs.stack, sizeof(storage_union)) == 0;
        }
    };
...
namespace std
{
    inline void swap(sxx::any& lhs, sxx::any& rhs) noexcept
    {
        lhs.swap(rhs);
    }
}
